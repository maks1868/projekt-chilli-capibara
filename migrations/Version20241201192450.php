 ...
 */
final class Version20241201192450 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE student_group (student_id INTEGER NOT NULL, group_id INTEGER NOT NULL, PRIMARY KEY(student_id, group_id), FOREIGN KEY (student_id) REFERENCES student (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY (group_id) REFERENCES "group" (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__course AS SELECT id, name FROM course');
        $this->addSql('DROP TABLE course');
        $this->addSql('CREATE TABLE course (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, name VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO course (id, name) SELECT id, name FROM __temp__course');
        $this->addSql('DROP TABLE __temp__course');
        $this->addSql('CREATE TEMPORARY TABLE __temp__department AS SELECT id, name FROM department');
        $this->addSql('DROP TABLE department');
        $this->addSql('CREATE TABLE department (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, name VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO department (id, name) SELECT id, name FROM __temp__department');
        $this->addSql('DROP TABLE __temp__department');
        $this->addSql('CREATE TEMPORARY TABLE __temp__group AS SELECT id, stationary, name FROM "group"');
        $this->addSql('DROP TABLE "group"');
        $this->addSql('CREATE TABLE "group" (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, stationary BOOLEAN NOT NULL, name VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO "group" (id, stationary, name) SELECT id, stationary, name FROM __temp__group');
        $this->addSql('DROP TABLE __temp__group');
        $this->addSql('CREATE TEMPORARY TABLE __temp__lesson AS SELECT id, course_id, group_id, teacher_id, room_id, start_time, end_time, type FROM lesson');
        $this->addSql('DROP TABLE lesson');
        $this->addSql('CREATE TABLE lesson (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, course_id INTEGER NOT NULL, group_id INTEGER NOT NULL, teacher_id INTEGER NOT NULL, room_id INTEGER NOT NULL, start_time DATETIME NOT NULL, end_time DATETIME NOT NULL, type VARCHAR(100) NOT NULL, FOREIGN KEY (course_id) REFERENCES course (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY (group_id) REFERENCES "group" (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY (teacher_id) REFERENCES teacher (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, FOREIGN KEY (room_id) REFERENCES room (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO lesson (id, course_id, group_id, teacher_id, room_id, start_time, end_time, type) SELECT id, course_id, group_id, teacher_id, room_id, start_time, end_time, type FROM __temp__lesson');
        $this->addSql('DROP TABLE __temp__lesson');
        $this->addSql('CREATE INDEX IDX_F87474F3591CC992 ON lesson (course_id)');
        $this->addSql('CREATE INDEX IDX_F87474F3FE54D947 ON lesson (group_id)');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__room AS SELECT id, department_id, name FROM room');
        $this->addSql('DROP TABLE room');
        $this->addSql('CREATE TABLE room (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, department_id INTEGER NOT NULL, name VARCHAR(255) NOT NULL, FOREIGN KEY (department_id) REFERENCES department (id) ON UPDATE CASCADE ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO room (id, department_id, name) SELECT id, department_id, name FROM __temp__room');
        $this->addSql('DROP TABLE __temp__room');
        $this->addSql('CREATE INDEX IDX_729F519BAE80F5DF ON room (department_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__student AS SELECT id, student_id FROM student');
        $this->addSql('DROP TABLE student');
        $this->addSql('CREATE TABLE student (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, student_id INTEGER NOT NULL)');
        $this->addSql('INSERT INTO student (id, student_id) SELECT id, student_id FROM __temp__student');
        $this->addSql('DROP TABLE __temp__student');
        $this->addSql('CREATE TEMPORARY TABLE __temp__teacher AS SELECT id, name FROM teacher');
        $this->addSql('DROP TABLE teacher');
        $this->addSql('CREATE TABLE teacher (id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT NULL, name VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO teacher (id, name) SELECT id, name FROM __temp__teacher');
        $this->addSql('DROP TABLE __temp__teacher');
    }
}
